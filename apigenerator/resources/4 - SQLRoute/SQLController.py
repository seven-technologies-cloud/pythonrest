...
def sql_direct_get_route():
    ...
    result = execute_query({'HTTP_QUERY': request.environ.get('HTTP_QUERY')}, request.method)
    return result
...
def sql_direct_post_patch_route():
    # Routing request to /sql many methods #
    result = execute_query(request.json, request.method)
    return result
...
def sql_stored_procedure_post_route():
    result = execute_post_route_sql_stored_procedure(request.environ, request.json)
    return result