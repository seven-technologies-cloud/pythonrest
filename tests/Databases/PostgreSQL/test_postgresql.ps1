...
$PIP_LOG = "$env:TEMP\pip_install_postgres_api.log"
...
$API_LOG = "$env:TEMP\api_output_postgres.log"
$API_LOG_ERROR = "$env:TEMP\api_error_output_postgres.log"
...
try {
    $response = Invoke-WebRequest -Uri "http://localhost:5000" -UseBasicParsing -OutFile "$env:TEMP\curl_check_postgres.log"
    ...
    if ($_.Exception.Response.StatusCode.value__ -eq 404 -or $_.Exception.Response.StatusCode.value__ -eq 400) {
        ...
        Write-Log "ERROR: API failed to start or is not responding. Curl check failed. See $env:TEMP\curl_check_postgres.log."
        if (Test-Path "$env:TEMP\curl_check_postgres.log") {
            Get-Content "$env:TEMP\curl_check_postgres.log"
        } else {
            Write-Log "curl_check_postgres.log not found. The API did not respond or failed to start."
        }
...
Remove-Item "$env:TEMP\curl_check_postgres.log" -ErrorAction SilentlyContinue

# 13. Teste GET
Write-Log "Performing sample GET request to http://localhost:5000/swagger..."
Invoke-WebRequest -Uri "http://localhost:5000/swagger" -UseBasicParsing -OutFile "$env:TEMP\curl_test_postgres.log"
if ($LASTEXITCODE -ne 0) {
    Write-Log "WARNING: Sample GET request failed. This might indicate an issue or no default route. See $env:TEMP\curl_test_postgres.log."
    Get-Content "$env:TEMP\curl_test_postgres.log"
} else {
    Write-Log "Sample GET request successful. Response logged to $env:TEMP\curl_test_postgres.log."
}
