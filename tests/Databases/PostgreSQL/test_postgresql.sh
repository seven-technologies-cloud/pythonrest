...
# 1. Log script start
log "Starting PostgreSQL integration test script."

# 2. Start PostgreSQL Docker container
log "Starting PostgreSQL Docker container..."
docker-compose up -d
if [ $? -ne 0 ]; then
    log "ERROR: Failed to start PostgreSQL Docker container."
    exit 1 # Trap will handle venv deactivation if it was activated
fi
log "PostgreSQL Docker container started."

# 3. Wait for PostgreSQL container to be healthy
PG_CONTAINER_NAME="pythonrest-postgres-1" # Adjust if your container name is different in docker-compose.yml
log "Waiting for PostgreSQL container ($PG_CONTAINER_NAME) to be healthy..."
TIMEOUT_SECONDS=120
SECONDS_WAITED=0
while true; do
    STATUS=$(docker inspect --format='{{.State.Health.Status}}' $PG_CONTAINER_NAME 2>/dev/null || echo "checking")
    if [ "$STATUS" == "healthy" ]; then
        log "PostgreSQL container is healthy."
        break
    fi
    if [ "$STATUS" == "unhealthy" ]; then
        log "ERROR: PostgreSQL container is unhealthy. Exiting."
        docker-compose logs $PG_CONTAINER_NAME
        docker-compose down
        exit 1 # Trap will handle venv deactivation
    fi
    if [ $SECONDS_WAITED -ge $TIMEOUT_SECONDS ]; then
        log "ERROR: Timeout waiting for PostgreSQL container to become healthy. Last status: $STATUS. Exiting."
        docker-compose logs $PG_CONTAINER_NAME
        docker-compose down
        exit 1 # Trap will handle venv deactivation
    fi
    ...
    log "Still waiting for PostgreSQL container... ($SECONDS_WAITED/$TIMEOUT_SECONDS seconds)"
done
...
python "$PROJECT_ROOT/pythonrest.py" generate --postgres-connection-string "postgresql://postgres:postgres@localhost:5432/database_mapper_postgresql?options=-c%20search_path=database_mapper_postgresql,public" > /tmp/pythonrest_generate_pg.log 2>&1
GENERATION_STATUS=$?
if [ $GENERATION_STATUS -ne 0 ]; then
    log "ERROR: PythonREST generation failed. See /tmp/pythonrest_generate_pg.log for details."
    cat /tmp/pythonrest_generate_pg.log
    # Deactivation will be handled by trap
    docker-compose down
    exit 1
fi
log "PythonREST generation output logged to /tmp/pythonrest_generate_pg.log."
...
pip install -r requirements.txt > /tmp/pip_install_pg_api.log 2>&1
INSTALL_STATUS=$?
if [ $INSTALL_STATUS -ne 0 ]; then
    log "ERROR: Failed to install dependencies for generated API. See /tmp/pip_install_pg_api.log for details."
    cat /tmp/pip_install_pg_api.log
...
log "Dependencies for generated API installed successfully. Output logged to /tmp/pip_install_pg_api.log."

# 11. Start the Flask API in the background
log "Starting Flask API in the background..."
python app.py > /tmp/api_output_pg.log 2>&1 &
API_PID=$!
log "Flask API started with PID $API_PID. Output logged to /tmp/api_output_pg.log."

# 12. Wait for API to start and check
log "Waiting for API to start (5 seconds)..."
sleep 5
curl -f http://localhost:5000 > /tmp/curl_check_pg.log 2>&1
CURL_STATUS=$?
if [ $CURL_STATUS -ne 0 ]; then
    log "ERROR: API failed to start or is not responding. Curl check failed. See /tmp/curl_check_pg.log."
    cat /tmp/curl_check_pg.log
    log "API logs (/tmp/api_output_pg.log):"
    cat /tmp/api_output_pg.log
    kill $API_PID
    deactivate # Deactivate generated API venv
    log "Virtual environment for generated API deactivated."
    cd .. # Back to $SCRIPT_DIR
...
rm /tmp/curl_check_pg.log # Clean up successful check log
...
curl http://localhost:5000/ > /tmp/curl_test_pg.log 2>&1
if [ $? -ne 0 ]; then
    log "WARNING: Sample curl GET request failed. This might indicate an issue or no default route. See /tmp/curl_test_pg.log."
    cat /tmp/curl_test_pg.log
else
    log "Sample curl GET request successful. Response logged to /tmp/curl_test_pg.log."
fi
...
# 17. Deactivate Shared PythonREST Venv (explicitly)
...
log "Stopping and removing PostgreSQL Docker container..."
docker-compose down
if [ $? -ne 0 ]; then
    log "WARNING: docker-compose down command failed. Manual cleanup might be required."
fi
log "PostgreSQL Docker container stopped and removed."
...
trap - EXIT
log "PostgreSQL integration test script completed successfully."
exit 0
