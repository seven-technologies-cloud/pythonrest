def get_domain_imports(domain_dict):
    domain_imports = ''
    constraint_names = list(set([constraint['referenced_class_name'] for constraint in domain_dict['Constraints']]))

    for constraint in constraint_names:
        if constraint == domain_dict['ClassName']:
            continue
        domain_imports = domain_imports + f'from src.c_Domain.{constraint} import *\n'
...
def get_sa_columns(domain_dict):
    ...
    for column in columns:
        try:
            result_string = result_string + tab + column['name'] + ": " + column['python_type'].replace('bytes', 'str') + ' = sa.Column(sa.' + column['sa_type'] + get_column_arguments_string(column) + ')\n'
        except Exception as e:
            print(domain_dict, e)
            return

    for constraint in constraints:
        result_string = result_string + tab + constraint['name'] + ": " + constraint['python_type'].replace('bytes', 'str') + ' = sa.Column(sa.' + constraint['sa_type'] + get_constraint_argument(constraint, domain_dict['TableName']) + get_column_arguments_string(constraint) + ')\n'
...
def get_self_columns(domain_dict):
    ...
    for column in columns:
        if column['python_type'] == 'bytes':
            self_columns_str = self_columns_str + tab + 'self.' + column['name'] + ' = ' + 'str.encode(' + column['name'] + ') if ' + column['name'] + ' else ' + column['name'] + '\n'
        else:
            self_columns_str = self_columns_str + tab + 'self.' + column['name'] + ' = ' + column['name'] + '\n'
